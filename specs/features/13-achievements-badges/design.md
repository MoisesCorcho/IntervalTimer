...
- Modelo `Achievement { id, title, description, iconAsset, condition }` definido como catalogo estatico en codigo/JSON.
- Modelo `UnlockedAchievement { achievementId, unlockedAt }` persistido localmente, inmutable una vez creado.
- `AchievementEvaluator` se ejecuta tras cada `onSessionEnd` (mismo hook que F04) comparando estado actual vs catalogo.
