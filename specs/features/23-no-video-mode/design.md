...
- Flag global `lowMediaMode: bool` en configuracion de usuario, consumido por el widget de renderizado de `Exercise` (F03) para decidir que variante mostrar.
- Cada `Exercise` (F03) ya debe tener un `imagePath` de fallback (definido en F03), este modo simplemente lo prioriza por eleccion del usuario en vez de por fallo tecnico.
