# Design: Ajuste Rapido de Intensidad
...
- Funcion pura `scaleRoutine(Routine, double factor) -> Routine` sin efectos secundarios, reutilizable en preview y ejecucion.
- UI: modal 'Ajustar intensidad' accesible desde la pantalla previa a iniciar cualquier rutina.
- Persistencia opcional: si el usuario confirma 'guardar cambios', se actualiza la rutina custom (solo aplica a rutinas propias, no a presets).
