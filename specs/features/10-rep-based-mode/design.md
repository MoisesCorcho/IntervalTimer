...
- Extiende `Interval` con `mode(time/reps)` y `targetReps: int?`.
- El `TimerController` (F01) trata los intervalos 'reps' como de duracion indefinida hasta un evento externo `onRepsConfirmed`.
- UI de ejecucion muestra boton grande 'Listo' en vez de barra de progreso para estos intervalos.
