...
- Exportacion CSV via generacion simple de texto delimitado desde `SessionLogRepository` (F04).
- Backup completo JSON serializando todas las tablas relevantes (`Routine`, `Interval`/`Block`, `SessionLog`, `BodyMeasurement`, configuracion).
- Paquete `share_plus`/`file_picker` para exportar/importar el archivo manualmente.
- Backup nativo de OS se logra automaticamente si la base de datos local (drift/isar) se almacena en el directorio estandar respaldado por el sistema (verificar en Android/iOS: `getApplicationDocumentsDirectory` suele estar incluido en backups nativos).
