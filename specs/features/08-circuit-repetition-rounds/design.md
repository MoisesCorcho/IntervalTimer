...
- Nuevo modelo `Block { id, intervals: List<Interval>, rounds: int }`; `Routine.items` pasa a ser `List<RoutineItem>` donde `RoutineItem = Interval | Block` (union type).
- El `TimerController` (F01) se extiende para 'aplanar' la rutina en tiempo de ejecucion en una secuencia lineal de pasos con metadata de ronda, manteniendo el resto de la logica sin cambios.
- UI del builder (F05) agrega accion 'Agrupar en bloque' sobre una seleccion multiple de intervalos.
