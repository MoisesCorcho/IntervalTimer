...
- Configuracion de audio session: iOS via `AVAudioSession` (categoria `.playback` con opcion `.duckOthers`), Android via `AudioManager` (`AUDIOFOCUS_GAIN_TRANSIENT_MAY_DUCK`).
- Paquete `audio_session` para abstraer la configuracion multiplataforma antes de cada anuncio de `VoiceAnnouncer` (F02).
- No requiere reproducir musica propia; solo gestiona el foco de audio para permitir convivencia con apps externas.
